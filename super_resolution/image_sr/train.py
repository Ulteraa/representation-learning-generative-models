...
import model_architecture, Perceptual_loss, dataset, config
...
def main():
    ...
    gen = model_architecture.Generator(config.IMAG_CHANEL).to(config.Device)
    dis = model_architecture.Discriminator(config.IMAG_CHANEL).to(config.Device)
